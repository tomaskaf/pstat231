...
prob.training = as.factor(ifelse(predict(model.glm, type="response")>0.5,"Yes","No"))

trainError<- calc_error_rate(prob.training,train$s33)
trainError
testError<- calc_error_rate(prob.training,test$s33)
testError



prob.training = NULL
prob.training =predict(model.glm)
pred = prediction(prob.training, train.cv$s33)
...
model.glm.cv<-cv.glm(test.cv,model.glm,cost,100)
model.glm.cv$delta
...
prob.training = as.factor(ifelse(predict(model.glm, type="response")>0.5,"Yes","No"))
confusionMatrix(data = prob.training, reference = train.cv$s33)
...
treeS33 <- tree(s33 ~ ., data=train.cv)
summary(treeS33)
```


```{r,echo=T}
cv <- cv.tree(treeS33, K=10, FUN = prune.misclass)
best.cv = min(cv$size[cv$dev == min(cv$dev)])

pt.cv = prune.misclass (treeS33, best=best.cv)
#draw.tree(pt.cv,nodeinfo = T)
...
prob.training = predict(pt.cv, test.cv, type="class")


confusionMatrix(data = prob.training, reference = test.cv$s33)
```
```